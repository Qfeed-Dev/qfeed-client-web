...
const ButtonQfeedSelect = ({ ...props }: toggleProps) => {
    return (
        <SelectWrapper gap={24}>
            <ButtonWrapper>
                <Input type="qfeed" id="친구가 만든 큐피드" />
                <Input type="qfeed" id="친구를 선택한 큐피드" />
                <Button
                    id="친구가 만든 큐피드"
                    state={
                        props.value === "친구가 만든 큐피드"
                            ? "active"
                            : "disabled"
                    }
                    onClick={props.onClick}
                >
                    <Text typo="Subtitle2b">친구가 만든 큐피드</Text>
                </Button>
                <Button
                    id="친구를 선택한 큐피드"
                    state={
                        props.value === "친구를 선택한 큐피드"
                            ? "active"
                            : "disabled"
                    }
                    onClick={props.onClick}
                >
                    <Text typo="Subtitle2b">친구를 선택한 큐피드</Text>
                </Button>
            </ButtonWrapper>
        </SelectWrapper>
    );
